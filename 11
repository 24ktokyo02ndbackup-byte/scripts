if getgenv().script_key == "Trial" then
print("Key Validated.")
else 
...
--// Track patched tools (weak table = auto cleanup)
...
local function applyRapidFire(tool)
    ...
    for _, conn in ipairs(getconnections(tool.Activated)) do
        ...
        for i = 1, info.nups do
            ...
            -- only touch realistic delay values
...
local function createESP(player)
    ...
    if ESPs[player] then return end -- Already has ESP
...
RunService.RenderStepped:Connect(function()
    if not ESPEnabled then
        for _, text in pairs(ESPs) do
            text.Visible = false
        end
        return
    end

    for player, text in pairs(ESPs) do
        local char = player.Character
        local head = char and char:FindFirstChild("Head")
        if head then
            local pos, onScreen = Cam:WorldToViewportPoint(head.Position + Vector3.new(0, 5, 0))
            if onScreen then
...
RunService.Heartbeat:Connect(function()
    local lpHum = Client.Character and Client.Character:FindFirstChildOfClass("Humanoid")

    -- Reset camera if spectate OFF or not locked
    if not getgenv().Zero.Ragebot.FollowTarget.Spectate or not LockedOn then
...
local function IsKO(character, healthLimit)
    ...
    -- Health fallback only (ignore ragdoll/physics)
...
local function GetSilentTarget()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= Client and player.Character then
            ...
            -- Wall check
            if getgenv().Zero.Silent.Checks.Wall and not WallCheck(hrp.Position, {Client, char}) then
                continue
            end
...
--// TRACER LINE FOR RAGEBOT
local TracerLine = Drawing.new("Line")
TracerLine.Thickness = 0.5
...
-- Update CachedRageCF every frame
RunService.RenderStepped:Connect(function()
    CachedRageCF = nil
...
-- Update CachedSilentCF every frame
RunService.RenderStepped:Connect(function()
    CachedSilentCF = nil

    if Zero.Silent.Enabled then
...
mt.__index = function(self, key)
    if self == Mouse and (key == "Hit" or key == "hit") then
        -- Ragebot has priority
         if UseSpecialRagebotHC() then
            return oldIndex(self, key)
        end
        if UseSpecialRagebotDH() then
            return oldIndex(self, key)
        end
...
-- Restore readonly to true
setreadonly(mt, true)

local wallCheckTick = 0
local WALL_CHECK_INTERVAL = 0.05 -- 20 checks/sec (more than enough)
...
RunService.Heartbeat:Connect(function()
    ...
    -- FORCEFIELD CHECK FIRST (cheap)
   

    -- WALL CHECK (throttled)
...
RunService.Heartbeat:Connect(function()
    if not UseSpecialRagebotDH() then return end
    if not LockedOn then return end
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if LockedOn then
        ...
        if target then
            if getgenv().Zero.Ragebot.Others.NotificationsEnabled then
                library:notification({text = "Locked on to: "..game.Players:FindFirstChild(tostring(target)).Name, time = 2.5})
            end
            LockedOn = true
...
-- put this OUTSIDE heartbeat (once)
local strafeAngle = 0
local heightOffset = 0
local speed = 20 -- strafe speed
local lerpSpeed = 0.3
...
RunService.Heartbeat:Connect(function()
    ...
    if getgenv().Zero.Ragebot.FollowTarget.Mode == "Random" then
		...
		if UseSpecialRagebotHC() then 
            myHRP.CFrame = CFrame.new(targetHRP.Position + Vector3.new(
    math.random(-getgenv().Zero.Ragebot.FollowTarget.Radius, getgenv().Zero.Ragebot.FollowTarget.Radius), -- X offset
     7,                   
    math.random(-getgenv().Zero.Ragebot.FollowTarget.Radius, getgenv().Zero.Ragebot.FollowTarget.Radius)  -- Z offset
		))
    else 
        myHRP.CFrame = CFrame.new(targetHRP.Position + Vector3.new(
    math.random(-getgenv().Zero.Ragebot.FollowTarget.Radius, getgenv().Zero.Ragebot.FollowTarget.Radius), -- X offset
...
style.items.sgui.Enabled = false
				local watermark = library:watermark({default = os.date('Zero - %b %d %Y - %H:%M:%S')})  

				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date('Zero - %b %d %Y - %H:%M:%S'))
					end 
				end) 
...
holder.items.sgui.Enabled = false
			...
			-- playerlist 
				...
				section:dropdown({name = "Target", items = {"Kill", "Teleport", "Neutral"}, default = "Neutral", flag = "PLAYERLIST_DROPDOWN", callback = function(text)
					library.prioritize(text)
                    if text == "Teleport" and game.Players:FindFirstChild(library.selected_player) then 
                        lp.Character.HumanoidRootPart.CFrame = game.Players:FindFirstChild(library.selected_player).Character.HumanoidRootPart.CFrame
                    end
                    if text == "Kill" and game.Players:FindFirstChild(library.selected_player) then 
                        pcall(function()
                         local targetPlayer = game.Players:FindFirstChild(library.selected_player)
    if not targetPlayer or not targetPlayer.Character then return end


    -- 2️⃣ Unequip unwanted tools
    local tool = game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool")
    if tool and not (tool.Name == guns['Double Barrel'] or tool.Name == guns['Shotgun'] or tool.Name == guns['Tactical Shotgun'] or tool.Name == guns['Revolver']) then
        tool.Parent = game.Players.LocalPlayer.Backpack
    end

    task.wait(0.05)
    -- enable ragebot for killing
    -- save settings
local saved = {
    Enabled = getgenv().Zero.Ragebot.Enabled,
    Autofire = getgenv().Zero.Ragebot.Autofire,
    FollowEnabled = getgenv().Zero.Ragebot.FollowTarget.Enabled,
    FollowRadius = getgenv().Zero.Ragebot.FollowTarget.Radius
}

local function restoreSettings()
    getgenv().Zero.Ragebot.Enabled = saved.Enabled
    getgenv().Zero.Ragebot.Autofire = saved.Autofire
    getgenv().Zero.Ragebot.FollowTarget.Enabled = saved.FollowEnabled
    getgenv().Zero.Ragebot.FollowTarget.Radius = saved.FollowRadius
...
getgenv().Zero.Ragebot.FollowTarget.Enabled = true 
getgenv().Zero.Ragebot.FollowTarget.Radius = 5
    -- 3️⃣ Select a gun
    local function getGun()
        return
            game.Players.LocalPlayer.Character:FindFirstChild(guns['Aug']) 
            or game.Players.LocalPlayer.Character:FindFirstChild(guns['Double Barrel'])
            or game.Players.LocalPlayer.Character:FindFirstChild(guns['Double Barrel For Other Games'])
            or game.Players.LocalPlayer.Character:FindFirstChild(guns['Shotgun'])
            or game.Players.LocalPlayer.Character:FindFirstChild(guns['Tactical Shotgun'])
            or game.Players.LocalPlayer.Character:FindFirstChild(guns['Revolver'])
            or game.Players.LocalPlayer.Backpack:FindFirstChild(guns['Aug'])
            or game.Players.LocalPlayer.Backpack:FindFirstChild(guns['Double Barrel'])
            or game.Players.LocalPlayer.Backpack:FindFirstChild(guns['Double Barrel For Other Games'])
            or game.Players.LocalPlayer.Backpack:FindFirstChild(guns['Shotgun'])
            or game.Players.LocalPlayer.Backpack:FindFirstChild(guns['Tactical Shotgun'])
            or game.Players.LocalPlayer.Backpack:FindFirstChild(guns['Revolver'])
    end
   ...
   task.spawn(function()
    ...
    local TIMEOUT = 6 -- seconds max before auto-restore
    ...
    Players.LocalPlayer.Character.HumanoidRootPart.CFrame = targetHRP.CFrame + Vector3.new(0, 5, 0)
    ...
    while LockedOn do
        ...
        -- success
        if targetPlayer.Character and IsKO(targetPlayer.Character, getgenv().Zero.Ragebot.Checks.Health_limit) then
            success = true
            break
        end

        -- manual unlock / timeout / invalid target
		...
		function library:colorpicker(options)
			...
			function cfg.set_visible(bool)
				...
				if bool then 
					if library.current_element_open and library.current_element_open ~= cfg then 
						library.current_element_open.set_visible(false)
						library.current_element_open = false 
					end
...
function cfg.create_player(player)
    -- normalize to Player instance
    if typeof(player) == "string" then
        player = players:FindFirstChild(player)
        if not player then return end
    end

    library.playerlist_data[player.Name] = {}
    local path = library.playerlist_data[player.Name]
    ...
    -- store real username
    player_name:SetAttribute("Username", player.Name)
				...
				TextButton.MouseButton1Click:Connect(function()
					...
					library.selected_player = player_name:GetAttribute("Username")

					library.config_flags["PLAYERLIST_DROPDOWN"](path.priority_text.Text)

					if cfg.labels.name then 
						local username = player_name:GetAttribute("Username")
local plr = players[username]

cfg.labels.name.set("User: " .. plr.Name)
cfg.labels.display.set("Display Name: " .. plr.DisplayName)
cfg.labels.uid.set("User Id: " .. plr.UserId)
			...
			function cfg.search(text)
				for _, player in next, players:GetPlayers() do 
					local name = tostring(player.DisplayName)
...
local Others = window:tab({ name = "Others" })
...
local Rage = colL:section({ name = "Ragebot" })
...
Rage:toggle({
    ...
    key = getgenv().Zero.Ragebot.Keybind,
    callback = function()
        getgenv().Zero.Ragebot.Keybind = library.flags["rage_enabled_bind"].key
    end
})
...
local Silent = colR:section({ name = "Silent Aim" })
...
local Self = colR:section({ name = "Self" })
...
Self:toggle({
    ...
    name = "Speed Key",
    flag = "cf_key",
    key = getgenv().Zero.Self.CFrameSpeed.Keybind,
    callback = function()
        getgenv().Zero.Self.CFrameSpeed.Keybind = library.flags["cf_key"].key
    end
})
...
local Follow = colL:section({ name = "Follow Target" })
...
local colO = Others:column()
local colR = Others:column()
local OtherSec = colO:section({ name = "Other" })

OtherSec:label({name = "Armor Keybind"}):keybind({
    name = "Get Armor Key",
    flag = "Armor_key",
    key = getgenv().Zero.Otherslol.Armorkeybind,
    callback = function()
        getgenv().Zero.Ragebot.Keybind = library.flags["Armor_key"].key
    end
})
...
local colV = Visuals:column()
local VisSec = colV:section({ name = "ESP" })
